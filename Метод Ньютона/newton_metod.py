...
from sympy import diff, symbols, E, sympify
...
class ConstantNamespace:
    ...
    TARGET_FUNC: str = 'x0 ** 2 + 2 * (x1 ** 2) + E ** (x0 + x1)'  # НЕТ ГЛОБАЛЬНОГО МИНИМУМА
...
class NewtonMethod:
    ...
    def n_run(self, index: int):
        ...
        if cond(calc_hess) <= 0:
            ...
            self.vector = self.vector - h * np.array(self.gradient)
        else:
            self.vector = np.array(self.vector) - np.dot(inv(calc_hess), self.gradient)
...
if __name__ == '__main__':
    ...
    s1 = NewtonMethod(constants.DIMENSION, constants.ACCURACY, constants.START_POINT)
    s1.newton()
